...
/******************************************/
/*        StakeSwap starts here       */
/******************************************/

pragma solidity 0.6.12;

contract StakeSwap is Ownable {
    using SafeMath for uint256;
    ...
    struct UserInfo 
    {
        uint256 amount;                     // How many GBT tokens the user has provided.
    ...
    uint256 public stakedGbts;
    PoolInfo public poolInfo;
    bool public emergencyActive;

    mapping (address => UserInfo) public userInfo;     // Info of each user that stakes GBT tokens.
    ...
    constructor(IERC20 _GBT, IERC20 _REWARD, uint256 _rewardPerBlock, uint256 _startBlock, uint256 _lockBlock, uint256 _endBlock) public {
        require(address(_GBT) != address(0), "_GBT address not set!");
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > poolInfo.lastRewardBlock && stakedGbts != 0) {
            uint256 multiplier = getMultiplier(poolInfo.lastRewardBlock, block.number);
            uint256 tokenReward = multiplier.mul(rewardPerBlock);
            accRewardPerShare = accRewardPerShare.add(tokenReward.mul(1e12).div(stakedGbts));
        }
    ...
    function updatePool() public {
        ...
        uint256 multiplier = getMultiplier(poolInfo.lastRewardBlock, block.number);
        uint256 tokenReward = multiplier.mul(rewardPerBlock);
        poolInfo.accRewardPerShare = poolInfo.accRewardPerShare.add(tokenReward.mul(1e12).div(stakedGbts));
        poolInfo.lastRewardBlock = block.number;
    }

    /**
     * @dev Deposit GBT tokens to Faucet for rewards allocation and/or withdraw outstanding rewards.
     * @param _amount Amount of GBT tokens to deposit.
     */
    function transact(uint256 _amount) public {
        ...
        if (block.number < lockBlock && _amount != 0) {
            GBT.safeTransferFrom(address(msg.sender), address(this), _amount);
            stakedGbts = stakedGbts.add(_amount);
            user.amount = user.amount.add(_amount);
            user.rewardDebt = user.amount.mul(poolInfo.accRewardPerShare).div(1e12);
            emit Deposit(msg.sender, _amount);
        }
        if (block.number >= endBlock) {
            GBT.safeTransfer(burnAddress, user.amount);
            stakedGbts = stakedGbts.sub(user.amount);
            user.amount = 0;
            user.rewardDebt = user.amount.mul(poolInfo.accRewardPerShare).div(1e12);
    ...
    function activateEmergency(address _to) external onlyOwner returns(bool) {
        ...
        if (GBT == REWARD) {
            return REWARD.transfer(_to, REWARD.balanceOf(address(this)) - stakedGbts);
        } else {
    ...
    function emergencyWithdraw() external {
        ...
        UserInfo storage user = userInfo[msg.sender];
        GBT.safeTransfer(address(msg.sender), user.amount);
        stakedGbts = stakedGbts.sub(user.amount);
