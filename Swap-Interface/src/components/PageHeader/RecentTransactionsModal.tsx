    ...
    <Modal title={TranslateString(1202, 'Recent transactions')} onDismiss={onDismiss}>
      {!account && (
        <Flex justifyContent="center" flexDirection="column" alignItems="center">
          ...
          <Button variant="tertiary" scale="sm" onClick={onDismiss} className="btn_yellow">
            Close
          </Button>
        </Flex>
      )}
      {account && chainId && sortedRecentTransactions.length === 0 && (
        <Flex justifyContent="center" flexDirection="column" alignItems="start">
          <Text mb="8px" bold>
            No recent transactions
          </Text>
          <Button className="btn_yellow btn-block-100" variant="tertiary" scale="sm" onClick={onDismiss}>
            Close
          </Button>
        </Flex>
      )}
